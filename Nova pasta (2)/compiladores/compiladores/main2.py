...
def exec_afd(automato, estado_atual,listaSimbolos):
    ...
    for c in listaSimbolos:
          ...
          if temRegra == False and estado_atual in automato.estadosFinais:
              print("Sintaxe correta token de ID : ",c.id," TOKEN: ",c.token,"  TIPO: ",c.tipo," LINHA: ",c.linha)
              estado_atual = "q0"
          elif temRegra == False and estado_atual not in automato.estadosFinais :
...
file_automato = open("compiladores\\compiladores\\automato_sintax.txt", 'r')
